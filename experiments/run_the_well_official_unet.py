#!/usr/bin/env python3
"""The Well 1.2 classic U-Net under the frozen sparse early-40 protocol."""
...
def evaluate(model, cases, scale, device, batch_size):
    ...
    for case in cases:
        ...
        prediction = torch.cat(predictions); target = case["target"]
...
def main():
    ...
    parser.add_argument("--output", type=Path,
                        default=Path("papers/longterm_results/the_well_official_unet_selection.json"))
    ...
    model = WellUNetClassic(5, 1, args.init_features).to(device)
    ...
    result = {
        "experiment_id": f"B-WELL-EARLY40-THE-WELL-UNET-{args.evaluation_split.upper()}",
        "status": "SELECTION" if args.evaluation_split == "validation" else "POSTHOC_CONFIRMATION",
        "library": {"name": "the_well", "version": "1.2.0",
                    "model": "UNetClassic", "upstream": "PDEBench"},
        "config": vars(args),
    ...
    print(f"unet: {args.evaluation_split} macro NRMSE="
