...
> 足够增益；更重要的是所有方法 NRMSE≈1，paired 只解释约 1.64% 方差，因此
> early-40 整体改判为 REJECTED，不再称为外部正信号。conditional core-IV 主动采样
> 也输给随机。完整、较简洁
...
| 最强正信号 | 2% 观测下 NRMSE `0.125±0.015`，明显优于 CP、flat GP、wrong geometry | 仅 controlled synthetic 强机制结果 |
| 当前最大缺口 | 外部/公共数据证据不足；主动采样不可宣称 | 外部有效性未通过；The Well 所有模型基本无效 |
| 强 neural-operator 对照 | 不适用当前 controlled mechanism 主表 | 已补 FNO/U-Net，但任务绝对效果失败，不能作为正证据 |
| 与原始“不规则边界”含义的一致性 | 已测试后放弃该 claim | phase/path geometry 只部分一致，明确不宣称 boundary geometry |
| 投稿准备度 | **方法主线与压力测试成立，外部证据不足** | **未闭环：只有 synthetic 机制，公共验证失败** |
...
### 4.2 The Well early-40：统计排序稳定，但绝对效果失败
...
这组结果最初被误判为“modest external evidence”。重新按绝对效果审计后：paired 的
近似 explained variance（1−NRMSE²）只有 1.64%，相对 zero 的 MSE skill
约 2.89%，相对 time-scaled persistence 约 1.96%。因此所有模型都基本无效，逐 seed
显著性只说明无效模型之间的排序稳定，**不构成 Paper B 的外部正证据**。
...
明确机制 → matched geometry control → synthetic 强结果
         → mismatch failure boundary → public-data absolute-effect failure
```
...
相对改善只有 `0.63%`，FNO 参数量约为 paired 的 15.5 倍。The Well 1.2
UNetClassic 同协议重训为 `1.00174±0.00208`，paired 胜 9/10，单侧 Wilcoxon
`p=0.001953`；time-scaled persistence 为 `1.00160±0.00276`。但参数效率和 p-value
都不能弥补 NRMSE≈1 的绝对失败。6% 相关路径误差
...
FNO/U-Net 只完成了 baseline 审计，没有给出有效重建。下一 external task 必须先达到
macro NRMSE ≤0.8 且相对最强 trivial baseline 的 MSE skill ≥20%，才能进一步讨论
paired significance。该门槛在 early-40 被拒后冻结。
...
**已撤回。**十 seed 小幅胜 FNO 不足以抵消 NRMSE≈1；early-horizon task 已改判
REJECTED。phase CP 只保留为 controlled synthetic 候选，不具有外部确认。
...
2. Paper B 重新选择/设计能先通过绝对效果门槛的外部任务；
...
经过绝对效果复核，项目不是“两篇都已成立”：

> **Paper A：让 mode-specific physical/operator geometry 定义 Bayesian Tucker factor space。**

> **Paper B 候选：让 propagation-path geometry 通过显式空间—时间 phase pairing进入 functional CP；目前只有 synthetic 机制信号，外部有效性未通过。**

Paper B 不能再以 early-horizon The Well 为正证据，也不承担一般 boundary geometry claim。

两篇真正共享的核心思想是：

> “几何”必须明确限定为 operator geometry、boundary geometry 或 propagation-path
> geometry。当前 A 支持第一种；B 的第三种仍待有效外部数据确认；第二种已经测试并放弃。
