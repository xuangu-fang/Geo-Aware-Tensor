...
| 当前最大缺口 | 外部/公共数据证据不足 | 绝对误差仍接近 1；GINO/U-Net 尚未补齐 |
...
|---|---:|---:|---:|---:|
| Paired phase CP | **0.99175±0.00579** | 8/10 | 0.01367 | 23,040 |
| Official FNO 2.0 | 0.99808±0.00286 | — | — | 357,473 |

![Paper B official FNO confirmation](../longterm_results/the_well_official_fno_confirmation.png)

相对改善只有 `0.63%`，但 FNO 参数量约为 paired 的 15.5 倍。因此当前最可信的 claim
是小而稳定的 inductive-bias/efficiency 收益，不是大幅 SOTA。剩余缺口为 GINO/U-Net
和 source/path noise ablation。
...
保留为传播路径几何的专门化方法。官方 FNO/TFNO 已补齐，下一步只考虑：
...
FNO gate 已给出肯定但幅度很小的答案。补完 GINO/U-Net 后做 source/path noise
ablation；不再改 paired 方法结构。
...
1. Paper B GINO/U-Net 与 sanity baselines；
2. Paper B source/path noise ablation；
3. Paper A 外部数据适配或 controlled-mechanism 定位；
4. 两篇写作和主表清理；
5. 不再投入 irregular boundary method tuning。
