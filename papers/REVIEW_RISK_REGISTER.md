...
| R2 | 几何信息是 oracle，baseline 没拿到相同信息 | 高 | 致命 | oracle-matched monolithic baseline；estimated/noisy geometry 曲线 | OPEN |
...
| R7 | baseline 实现弱或未调优 | 高 | 致命 | 作者实现/官方库；版本 SHA；统一 tune budget；parameter+compute matched | OPEN |
...
| R11 | B 只在人工 aligned regime 有效 | 高 | 高 | 外部 wave data；mismatch phase diagram；明确 scope，不宣称通用 SOTA | PARTIAL |
